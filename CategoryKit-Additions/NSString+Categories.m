...
CategoryKit_LoadCategory(NSString_Categories)
...
- (NSInteger)stringLength {
    NSInteger gbkEncodingStringlength = 0;
    //  这里一定要使用gbk的编码方式，网上有很多用Unicode的，但是混合的时候都不行
    NSStringEncoding gbkEncoding =CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
    NSInteger stringlength = [self lengthOfBytesUsingEncoding:gbkEncoding];
    char *gbkEncodingString = (char *)[self cStringUsingEncoding:gbkEncoding];
    for (NSInteger i = 0; i < stringlength; i++) {
        if (gbkEncodingString) {
            gbkEncodingString++;
            gbkEncodingStringlength++;
        } else {
            gbkEncodingString++;
        }
    }
    return gbkEncodingStringlength;
}
...
- (NSString*)transformEmojiTextToText;{
    __block NSString *newString = [self copy];
...
- (NSArray *)stringBetweenCharactersSeparatedFrom:(NSString *)from to:(NSString *)to;{
    ...
    while (![scanner isAtEnd]) {
        ...
        if (stringBetweenBrackets) {
            @autoreleasepool {
                NSString * const StringBetweenCharactersWordsKey = @"StringBetweenCharactersWordsKey";
                NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
                [dic setObject:stringBetweenBrackets forKey:StringBetweenCharactersWordsKey];
                [dic setObject:[NSValue valueWithRange:NSMakeRange(fIndex, tIndex + 1 - fIndex)] forKey:StringBetweenCharactersWordsKey];
                [array addObject:dic];
            }
        }
    }
